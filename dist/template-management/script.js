...
const templateBaseApi = new URLSearchParams(window.location.search)
    .get("apiBaseUrl")
    ?.replace(/\/+$/, "");
...
function confirmSaveTemplate() {
    ...
    currentTemplateId
        ? `${templateBaseApi}/template/${currentTemplateId}`
        : `${templateBaseApi}/create-template`,
...
confirmDeleteBtn.addEventListener("click", async () => {
    ...
    const response = await fetch(`${templateBaseApi}/template/${id}`, {
        method: "DELETE"
    });
...
addTemplateBtn.addEventListener("click", () => {
    ...
    document.getElementById("renameContainer").innerHTML = "";
...
async function loadTemplates() {
    ...
    try {
        ...
        const response = await fetch(`${templateBaseApi}/templates`);
